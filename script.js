...
(() => {
  ...
  if (lightbox) {
    ...
    // Mark zoomable — interact via wrapper so pointer-events:none on img is preserved
    items.forEach((it, i) => {
      it.wrapper.classList.add("figure-zoom");
      it.wrapper.setAttribute("tabindex", "0");
      it.wrapper.setAttribute("role", "button");
      it.wrapper.setAttribute("aria-label", "Open image · " + (it.caption || it.alt));
      const open = (e) => {
        e.preventDefault();
        openLightbox(i);
      };
      it.wrapper.addEventListener("click", open);
      it.wrapper.addEventListener("keydown", (e) => {
  ...
  if (cursor && isFinePointer && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    ...
    const hoverSel = "a, button, .figure-zoom, [role='button'], .menu-btn, input, label";
  ...
  // Disable drag on all images (extra layer on top of CSS user-drag:none)
  $$("img").forEach((img) => img.setAttribute("draggable", "false"));
  // Block right-click context menu over any image or image container
  document.addEventListener("contextmenu", (e) => {
    if (e.target.tagName === "IMG" || e.target.closest("figure, .hero-img")) e.preventDefault();
  }, true);
  // Block drag-start on image containers (belt-and-suspenders with CSS)
